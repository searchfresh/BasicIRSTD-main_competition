...
parser.add_argument("--pth_dirs_1", default=r"checkpoint/Dataset-mask/LKUNet_hrall.pth",
                    help="checkpoint dir" )
...
parser.add_argument("--pth_dirs_3", default=r"checkpoint/Dataset-mask/LKUNet_69.pth",
                    help="checkpoint dir" )
parser.add_argument("--pth_dirs_4", default=r"checkpoint/Dataset-mask/LKUNet_lrall.pth",
                    help="checkpoint dir" )
...
def test():
    ...
    net4 = Net(model_name=opt.model_names_1, mode='test').cuda()
    if opt.SWA == True:
        # net1 = AveragedModel(net1)
        net2 = AveragedModel(net2)
        # net3 = AveragedModel(net3)
        # net4 = AveragedModel(net4)
    ...
    with torch.no_grad():
        for idx_iter, (img, size, img_dir, ori_size) in enumerate(test_loader):
            ...
            if opt.save_img == True:
                if ori_size[-1] > 1536 or ori_size[-2] > 1536:
                    img_save = transforms.ToPILImage()(((pred[0, 0, :, :] > 0.95).float()).cpu())
                else:
