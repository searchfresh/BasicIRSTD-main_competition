...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config=config, **kwargs):
        ...
        self.cls_head = nn.Sequential(
                            nn.Conv2d(ocr_mid_channels, ocr_mid_channels//4, kernel_size=1, stride=1, padding=0, bias=True),
                            nn.BatchNorm2d(ocr_mid_channels//4),
                            nn.ReLU(inplace=relu_inplace),
                            nn.Upsample(scale_factor=2,mode='bilinear',align_corners=ALIGN_CORNERS),
                            nn.Conv2d(ocr_mid_channels//4, ocr_mid_channels//8, kernel_size=1, stride=1, padding=0, bias=True),
                            nn.BatchNorm2d(ocr_mid_channels // 8),
                            nn.ReLU(inplace=relu_inplace),
                            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=ALIGN_CORNERS),
                            nn.Conv2d(ocr_mid_channels//8, NUM_CLASSES, kernel_size=1, stride=1, padding=0, bias=True))
    ...
    def forward(self, x):
        ...
        out_aux_seg.append(F.interpolate(out_aux,scale_factor=4,mode='bilinear',align_corners=ALIGN_CORNERS).sigmoid())
        out_aux_seg.insert(0,out.sigmoid())
