...
class SearchNetLoss(nn.Module):
    ...
    def forward(self, pred, gt_masks, idx_iter, idx_epoch):
        ...
        for i in range(self.layer_lvl_num-1):
            loss_search += self.bce(self.pool8(weighted_mask_1[i]).reshape(b, -1), self.pool2(gt_masks).reshape(b, -1))
            loss_search += self.bce(self.pool16(weighted_mask_2[i]).reshape(b, -1), self.pool4(gt_masks).reshape(b, -1))
            loss_search += self.bce(self.pool32(weighted_mask_3[i]).reshape(b, -1), self.pool8(gt_masks).reshape(b, -1))
